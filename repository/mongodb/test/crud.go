...
func (suite *PackageTestSuite) TestFind() {
	test1 := suite.makeTestStruct("test", "a", "b")
	_, err := suite.repo.Create(suite.ctx, test1)
	suite.NoError(err)
	test2 := suite.makeTestStruct("test2", "a", "b", "c")
	...
	typeGuide := suite.makeTestStruct("test2", "a", "b", "c")
	total, items, err := suite.repo.Find(suite.ctx, nil, typeGuide)
...
func (suite *PackageTestSuite) TestFilterFind() {
	test1 := suite.makeTestStruct("test", "a", "b")
	_, err := suite.repo.Create(suite.ctx, test1)
	suite.NoError(err)
	test2 := suite.makeTestStruct("test2", "a", "b", "c")
	...
	opt := &domain.QueryOption{
		Filters: []string{"title:eq:test2"},
	}
	total, items, err := suite.repo.Find(suite.ctx, opt, typeGuide)
...
func (suite *PackageTestSuite) TestSearchFindNotFound() {
	test1 := suite.makeTestStruct("test", "a", "b")
	_, err := suite.repo.Create(suite.ctx, test1)
	suite.NoError(err)
	test2 := suite.makeTestStruct("test2", "a", "b", "c")
